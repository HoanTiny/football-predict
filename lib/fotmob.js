...
// Cache riêng cho lịch đa giải theo ngày (trang "Hôm nay") — 30s: đủ tươi khi có trận live,
// đủ giãn để không dội FotMob khi nhiều người dùng cùng xem 1 ngày.
const scheduleCache = new Map();
const SCHEDULE_TTL = 1000 * 30;
async function fmSchedule(path) {
  const hit = scheduleCache.get(path);
  if (hit && hit.exp > Date.now()) return hit.data;
  const res = await fetch(BASE + path, {
    headers: { "User-Agent": UA, Accept: "application/json" },
    cache: "no-store",
  ...
  if (!res.ok) throw new Error(`FotMob HTTP ${res.status}`);
  const json = await res.json();
  scheduleCache.set(path, { exp: Date.now() + SCHEDULE_TTL, data: json });
  return json;
}

// Thứ tự hiển thị ưu tiên = thứ tự khai báo trong lib/leagues.js.
const CURATED_ORDER = new Map(LEAGUES.map((l, i) => [String(l.id), i]));

// 1 trận trong lịch theo ngày -> CÙNG SHAPE với normFixture của lib/fotmobLeagues.js
// (để dùng chung MatchRow + MatchDetailSheet cho cả 2 nguồn).
function normDailyMatch(m) {
  const st = m.status || {};
  const playing = !!st.started || !!st.finished;
  // Endpoint /data/matches?date= cho SẴN penScore riêng (m.home.score = tỉ số thường,
  // KHÔNG cộng pen) — chính xác hơn nhiều so với suy luận trừ như football-data.
  const hasPen = m.home?.penScore != null && m.away?.penScore != null;
  return {
    id: String(m.id),
    ...
    liveTime: st.liveTime?.short || null,
    pen: hasPen ? { home: m.home.penScore, away: m.away.penScore } : null,
    home: { id: m.home?.id ?? null, name: m.home?.name || m.home?.longName || "?", score: playing ? m.home?.score ?? null : null },
    away: { id: m.away?.id ?? null, name: m.away?.name || m.away?.longName || "?", score: playing ? m.away?.score ?? null : null },
  };
}

/**
 * Lịch + kết quả TẤT CẢ giải trong 1 ngày (kiểu trang chủ FotMob/Apple Sports).
 * Chỉ giữ các giải trong danh mục lib/leagues.js (tránh rác từ giải nhỏ/nghiệp dư), sắp xếp
 * theo đúng thứ tự khai báo ở đó. dateKeyStr dạng "YYYYMMDD" — do CLIENT tính theo giờ máy
 * (khớp phần chuyển sang giờ địa phương ở lib/time.js), server không tự đoán "hôm nay".
 *
 * QUAN TRỌNG: với giải chia bảng/vòng (World Cup, UCL…), FotMob trả NHIỀU bucket trong 1 ngày
 * (id khác nhau theo bảng/vòng, vd "World Cup Grp. K") nhưng CÙNG `primaryId` = id giải cha
 * (khớp id trong lib/leagues.js). Phải gộp theo `primaryId`, lọc theo `id` sẽ mất trắng các
 * giải này (đã verify: World Cup ngày 29/06/2026 có id=894789, primaryId=77).
 *
 * Trả { date, leagues: [{ id, name, ccode, matches:[…] }] }. Lỗi/rỗng -> leagues: [].
 */
export async function fotmobDailySchedule(dateKeyStr) {
  if (!dateKeyStr) return { date: null, leagues: [] };
  try {
    const data = await fmSchedule(`/data/matches?date=${dateKeyStr}`);
    const byPrimary = new Map(); // primaryId (string) -> match[]
    for (const lg of data?.leagues || []) {
      const pid = String(lg.primaryId ?? lg.id);
      if (!CURATED_ORDER.has(pid)) continue;
      const list = byPrimary.get(pid) || [];
      list.push(...(lg.matches || []).map(normDailyMatch));
      byPrimary.set(pid, list);
    }
    const leagues = [...byPrimary.entries()]
      .filter(([, matches]) => matches.length)
      .map(([pid, matches]) => {
        // Ưu tiên tên/mã đã chuẩn hoá trong lib/leagues.js (vd "World Cup 2026" thay vì "World Cup").
        const meta = LEAGUES.find((l) => String(l.id) === pid);
        return {
          id: pid,
          name: meta?.name || null,
          ccode: meta?.ccode || null,
          matches: matches.sort((a, b) => new Date(a.utcTime) - new Date(b.utcTime)),
        };
      })
      .sort((a, b) => (CURATED_ORDER.get(a.id) ?? 999) - (CURATED_ORDER.get(b.id) ?? 999));
    return { date: data?.date || dateKeyStr, leagues };
  } catch {
    return { date: dateKeyStr, leagues: [] };
  }
}
...
/**
 * Tra số tỉ số pen chính xác theo matchId, dùng endpoint /data/matches?date= (CÓ penScore) để
 * bù cho danh sách lấy từ /data/leagues?...&tab=fixtures (KHÔNG có penScore riêng, chỉ có cờ
 * statusShort="Pen"). Cùng hệ FotMob nên khớp thẳng qua id, không cần đoán theo tên đội.
 * Trả Map: matchId(string) -> { home, away }.
 */
export async function fotmobPenScoresByDate(dateKeys) {
  const map = new Map();
  for (const dk of new Set(dateKeys || [])) {
    let data;
    try {
      data = await fm(`/data/matches?date=${dk}`, MATCHES_TTL);
    } catch {
      continue;
    }
    for (const lg of data?.leagues || []) {
      for (const mt of lg.matches || []) {
        if (mt.home?.penScore != null && mt.away?.penScore != null) {
          map.set(String(mt.id), { home: mt.home.penScore, away: mt.away.penScore });
        }
      }
    }
  }
...
function buildEvents(content, homeName, awayName) {
  ...
  for (const e of arr) {
    ...
    if (e.type === "Goal") {
      ...
      out.push({ type: "Card", minute, team, isHome: !!e.isHome, player, detail: isRed ? "Red Card" : "Yellow Card" });
    }
  }
  return out;
}
...
function buildMatchDetail(data, id, fallbackHome, fallbackAway) {
  ...
  const headerStatus = data?.header?.status;
  const penalties = headerStatus?.reason?.penalties;
  const pen = Array.isArray(penalties) && penalties.length >= 2
  ...
  return {
    ...
    fixtureId: id,
    source: "fotmob",
    teamColors: data?.general?.teamColors || null,
    pen,
  };
}
...
function buildSide(rounds, bronze) {
  ...
  // Vòng knockout đầu: WC dùng stage "1/16"; UCL/UEL dùng "playoff" (vòng playoff 16 đội).
  const r32 = byStage["1/16"] || byStage["playoff"] || [];
 ...
 * Dùng được cho mọi giải cúp có playoff trên FotMob (WC 77, UCL 42, UEL 73…).
 */
export async function fotmobBracket(leagueId = WC_LEAGUE_ID) {
  const empty = { asItStands: null, confirmed: null, hasData: false, source: "fotmob" };
  try {
    const data = await fm(`/data/leagues?id=${leagueId}`, BRACKET_TTL);
 ...
 */
async function currentSeasonId(leagueId = WC_LEAGUE_ID) {
  try {
    const j = await fm(`/data/leagues?id=${leagueId}`, BRACKET_TTL);
 ...
 */
export async function fotmobTopStats(category = "goals", leagueId = WC_LEAGUE_ID) {
  const seasonId = await currentSeasonId(leagueId);
  if (!seasonId) return [];
  const path = `/stats/${leagueId}/season/${seasonId}/${category}.json`;
