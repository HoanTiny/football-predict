...
import { fotmobPenScoresByDate, dateKey } from "./fotmob";
...
function normFixture(m) {
  ...
  return {
    ...
    // reason.short: "FT" | "HT" | "AET" | "Pen"… hoặc phút hiện tại khi đang đá (ở liveTime).
    statusShort: st.reason?.short || null,
    liveTime: m.status?.liveTime?.short || null,
    // Endpoint fixtures KHÔNG có số tỉ số pen riêng (chỉ có cờ "Pen" ở statusShort) — để null,
    // MatchRow tự hiện badge "Kết thúc sau loạt luân lưu" gọn thay vì bịa số.
    pen: null,
...
export async function fotmobLeagueFixtures(leagueId) {
  ...
  try {
    ...
    // Bù số tỉ số pen chính xác cho các trận đi loạt luân lưu (endpoint fixtures không có sẵn) —
    // tra chéo qua /data/matches?date= của các ngày liên quan, khớp thẳng theo matchId.
    const penMatches = matches.filter((m) => m.finished && m.statusShort === "Pen");
    if (penMatches.length) {
      const dateKeys = penMatches.map((m) => dateKey(new Date(m.utcTime)));
      const penMap = await fotmobPenScoresByDate(dateKeys);
      if (penMap.size) {
        matches = matches.map((m) =>
          m.statusShort === "Pen" && penMap.has(m.id) ? { ...m, pen: penMap.get(m.id) } : m
        );
      }
    }
