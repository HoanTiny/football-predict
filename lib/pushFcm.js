...
/** Gửi 1 notification tới 1 token; trả true nếu thành công, ném lỗi kèm mã nếu thất bại. */
async function sendOne(token, payload, accessToken, projectId) {
...
async function sendToRows(rows, payload) {
  ...
  await Promise.all(
    rows.map(async (r) => {
      try {
        await sendOne(r.token, payload, accessToken, sa.project_id);
      } catch (e) {
