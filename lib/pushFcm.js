...
/** Như sendFcmDataToUserIds nhưng gửi TẤT CẢ token — dùng để test live-update thủ công. */
export async function sendFcmDataToAll(data) {
  if (!fcmReady()) return;
  const { data: rows } = await supabaseAdmin.from("fcm_tokens").select("token");
  await sendToRows(rows, { data }, true);
}
