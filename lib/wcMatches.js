...
function mapFixture(f) {
  ...
  // Hiệp phụ + loạt luân lưu (knockout) — để realScore() trong constants.js tách "PĐ x-y".
  const etObj = f.score?.extratime;
  const penObj = f.score?.penalty;
  const hasET = etObj?.home != null;
  const hasPen = penObj?.home != null;
  const duration = hasPen
    ? "PENALTY_SHOOTOUT"
    : hasET
      ? "EXTRA_TIME"
      : "REGULAR";
  ...
  return {
    ...
    score: {
      // api-football `goals` = sau 90'+ET, CHƯA cộng pen. Để realScore() (constants.js) dùng
      // CÔNG THỨC chung với football-data (fullTime đã bao gồm pen tích luỹ), ta cộng pen vào
      // fullTime khi có pen → giữ semantic thống nhất 1 chỗ.
      fullTime: hasPen
        ? { home: home + (penObj.home || 0), away: away + (penObj.away || 0) }
        : { home, away },
      regularTime: { home: rtHome, away: rtAway },
      extraTime: hasET
        ? { home: etObj.home || 0, away: etObj.away || 0 }
        : { home: 0, away: 0 },
      penalties: hasPen
        ? { home: penObj.home || 0, away: penObj.away || 0 }
        : null,
      duration,
      winner,
    },
