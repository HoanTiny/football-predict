...
/**
 * Đội hình xuất phát + dự bị + sơ đồ (chỉ có khi đội đã công bố — thường 30–60' trước trận).
 * Trả về: { home: {team, formation, coach, startXI:[{number,name,pos,grid}], substitutes}, away: {…} }
 * grid = "row:col" (1 = hàng trên cùng = thủ môn xa nhất tính từ khung thành). null nếu chưa có.
 */
export async function fixtureLineups(fixtureId) {
  if (!fixtureId) return null;
  // Cache ngắn 60s khi chưa có; sau khi đã có lineup thì cache lâu (lineup hiếm khi đổi).
  const data = await af(`/fixtures/lineups?fixture=${fixtureId}`, 1000 * 60);
  const arr = data?.response || [];
  if (arr.length < 2) return null;

  const norm = (t) => {
    const players = t.startXI || [];
    return {
  ...
  return { home: norm(arr[0]), away: norm(arr[1]) };
}
