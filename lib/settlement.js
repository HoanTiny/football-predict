...
/**
 * Hệ số thưởng kèo 1X2 (Thắng/Hòa/Thua): Hòa ×2; cược "cửa dưới" thắng ×3; cửa trên thắng ×1.
 */
export function wdlMultiplier(selection, homeTeam, awayTeam) {
  if (selection === "DRAW") return 2;
  const hr = getFifaRank(homeTeam);
  const ar = getFifaRank(awayTeam);
  if (hr == null || ar == null) return 1;
  // hạng số lớn hơn = yếu hơn (cửa dưới)
  if (selection === "HOME") return hr > ar ? 3 : 1;
  if (selection === "AWAY") return ar > hr ? 3 : 1;
  return 1;
}
