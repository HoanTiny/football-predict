...
export function computeSettlement(matches, player, alreadySettled) {
  ...
  const predictions = player.predictions.map((p) => {
    const pMatchIdStr = String(p.matchId);
    if (p.status !== "pending" || alreadySettled.has(pMatchIdStr)) return p;
    const m = finished.get(pMatchIdStr);
    if (!m) return p;
    settledMatchIds.push(pMatchIdStr);
    changed = true;
