...
/** Tra toạ độ một thành phố (Open-Meteo geocoding, miễn phí, không cần key). */
...
/**
 * Dự báo thời tiết tại sân vào ngày trận (Open-Meteo, miễn phí).
 * Ưu tiên toạ độ từ tên sân (bảng sân WC); nếu không có thì geocode theo thành phố.
 */
export async function getWeather({ venue, city, isoDate }) {
  let loc = venueLocation(venue);
  if (!loc && city) loc = await geocodeCity(city);
  if (!loc) return city ? { city } : null;
  if (!isoDate) return { city: loc.city };
