...
export async function insertPrediction(me, bet) {
  ...
  throwIf(e1);
  // Trừ chip NGUYÊN TỬ ở server (tránh lost-update do đọc-sửa-ghi từ client).
  const { error: e2 } = await supabase.rpc("adjust_chips", { p_id: me.id, p_delta: -bet.wager });
  throwIf(e2);
}
...
export async function applySettlementToRoom(me, result) {
  ...
  // Cộng chip NGUYÊN TỬ (không ghi đè bằng snapshot cũ → không mất tiền thắng).
  if (result.chipsGain) {
    const { error } = await supabase.rpc("adjust_chips", { p_id: me.id, p_delta: result.chipsGain });
    throwIf(error);
  }
  if (result.championPicks) {
    const { error } = await supabase
      .from("players")
      .update({ champion_picks: result.championPicks })
      .eq("id", me.id);
    throwIf(error);
  }
}
...
export async function addChampionPick(me, stage, team, wager, multiplier) {
  ...
  const { error: e1 } = await supabase
    .from("players")
    .update({ champion_picks: [...existing, newPick] })
    .eq("id", me.id);
  throwIf(e1);
  const { error: e2 } = await supabase.rpc("adjust_chips", { p_id: me.id, p_delta: -wager });
  throwIf(e2);
}
