...
/** Tạo phòng mới (kèm tên hiển thị tuỳ chọn + giải đấu), trả về mã phòng. */
export async function createRoom(name, leagueId) {
  const code = genRoomCode();
  const { error } = await supabase
    .from("rooms")
    .insert({ code, name: name?.trim() || null, league_id: leagueId || 77 });
  throwIf(error);
  return code;
}
...
export async function joinRoom(code, name, userId) {
  if (!userId)
    throw new Error("Yêu cầu đăng nhập tài khoản trước khi vào phòng.");

  const { data: room, error: roomErr } = await supabase
    .from("rooms")
    .select("code, name, league_id")
    .eq("code", code)
    .maybeSingle();
  throwIf(roomErr);
    ...
    if (Object.keys(patch).length > 0) {
      ...
      if (updErr) {
        if (updErr.code === "23505") {
          throw new Error(
            "Tên hiển thị này đã được sử dụng trong phòng. Vui lòng chọn tên khác!",
          );
        }
        throwIf(updErr);
      }
      return { ...updated, room_name: room.name, room_league_id: room.league_id };
    }
    return { ...existing, room_name: room.name, room_league_id: room.league_id };
  }

  // Không set chips ở client (chip dùng default phía DB = 5000) để tránh gian lận.
  const { data: created, error: insErr } = await supabase
    .from("players")
    .insert({ room_code: code, name, user_id: userId })
    .select()
    .single();
  if (insErr) {
  ...
  return { ...created, room_name: room.name, room_league_id: room.league_id };
}

/** Tải toàn bộ trạng thái phòng: người chơi (chưa rời) + mọi kèo. */
export async function fetchRoomState(code) {
  const [{ data: players, error: e1 }, { data: predictions, error: e2 }] =
    await Promise.all([
      supabase
        .from("players")
        .select("*")
...
export async function insertPrediction(me, bet) {
  const { error } = await supabase.rpc("place_bet", {
    p_room: me.room_code,
    p_match: bet.matchId,
    p_home: bet.homeGoals ?? null,
    p_away: bet.awayGoals ?? null,
    p_wager: bet.wager,
    p_kickoff: bet.kickoff || null, // giờ bóng lăn → RLS ẩn kèo người khác tới khi khoá
    p_type: bet.betType || "score",
    p_selection: bet.selection || null,
  });
  if (error) {
    if (/INSUFFICIENT_CHIPS/.test(error.message))
      throw new Error("Không đủ chip");
    if (/MATCH_LOCKED/.test(error.message))
      throw new Error("Trận đã bắt đầu — đã khóa cược");
    throwIf(error);
  }
}
...
export async function fetchUserRooms(userId) {
  ...
  const { data, error } = await supabase
    .from("players")
    .select("room_code, id, name, rooms(name, league_id)")
    .eq("user_id", userId)
    .is("left_at", null);
  throwIf(error);
  return (data || []).map((p) => ({
