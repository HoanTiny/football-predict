...
export async function insertPrediction(me, bet) {
  const { error } = await supabase.rpc("place_bet", {
    ...
    p_wager: bet.wager,
    p_kickoff: bet.kickoff || null, // giờ bóng lăn → RLS ẩn kèo người khác + khoá sau kickoff
  });
  if (error) {
    if (/INSUFFICIENT_CHIPS/.test(error.message)) throw new Error("Không đủ chip");
    if (/MATCH_LOCKED/.test(error.message)) throw new Error("Trận đã bắt đầu — đã khóa cược");
    throwIf(error);
  }
}
