...
export async function applySettlementToRoom(me, result) {
  ...
  if (result.championPick && result.championPick.status !== "pending") {
    patch.champion_status = result.championPick.status;
    patch.champion_payout = result.championPick.payout;
  }
  const { error } = await supabase.from("players").update(patch).eq("id", me.id);
  throwIf(error);
}

/** Chốt cược vô địch (một lần duy nhất). */
export async function setChampionPick(me, team, wager) {
  const { error } = await supabase
    .from("players")
    .update({
      chips: me.chips - wager,
      champion_team: team,
      champion_wager: wager,
...
/** Reset hồ sơ của tôi trong phòng: xoá kèo, chip về 1.000. */
export async function resetMyData(me) {
  const { error: e1 } = await supabase.from("predictions").delete().eq("player_id", me.id);
  throwIf(e1);
  const { error: e2 } = await supabase
    .from("players")
    .update({ chips: START_CHIPS, champion_team: null, champion_wager: null, champion_status: null, champion_payout: null })
    .eq("id", me.id);
  throwIf(e2);
}
